...
import { linebreakWithDashes } from "../markdown-components/linebreak.js";
...
export function AddToApiRoute(
  ...
  source =
    `---\noutline: [2,3]\n---\n${comment(
      ...
      // TODO: Add ability for TypeNodes to create types at the start of the page, the ApiPageScriptSetup does this but
      // the items aren't rendered as markdown, so ## h2 will not work and using stuff like codeblocks will cause vue to error.
      // needs fixing to renable this feature. "" ?? ___ should be removed obviously.
    )}\n${"" ?? ApiPageScriptSetup}\n` + source;
  fs.writeFileSync(
    path.join(
      TargetRoute,
...
/**
 * `<script>` source that is placed at the top of each api file, exposes functions like that Nodes can use to modify
 * the page they're one.
 */
const ApiPageScriptSetup = `
${comment(
  "API page setup script that allows interaction between docs nodes and the page they're on."
)}
<script>
    import { ref } from "vue";
    ...
    export default {
        setup(){
            const PageTypeDefinitions = ref([])
            
            // Make sure to wrap components that call this function in a "vue-once" component to prevent recursive rendering.
            function AddTypeDefinitionToPage(content){
              PageTypeDefinitions.value.push(content)
            }
            return {AddTypeDefinitionToPage,PageTypeDefinitions}
        }
    }
</script>
${comment(
  "type definitions for this page are generated by the TypeNode that are rendered."
)}
<template v-if="true">
  <h2>Types</h2>
  <div v-for="content in PageTypeDefinitions">{{ content }}</div>
  ${linebreakWithDashes()}
</template>
`;
